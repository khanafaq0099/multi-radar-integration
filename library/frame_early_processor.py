...
class FrameEProcessor(DataProcessor):  # early processing for frame of each radar before merging
    def __init__(self, **kwargs_CFG):
        ...
        self.FEP_frame_group_deque = deque([], FEP_CFG['FEP_frame_deque_length']) #10
    ...
    def FEP_accumulate_update(self, frame):  # ndarray(points, channels=5) of 1 frame
        # append frames Store the new frame
        self.FEP_frame_group_deque.append(frame)
        # Concatenate all frames currently in the deque
        ...
        #Return all accumulated (and processed) frames
...
if __name__ == '__main__':
    ...
    data_before = np.stack([x, y, z], axis=1)
    data_after = f.FEP_trans_rotation_3D(data_before)

    # -----------------------------
    # Plot before vs after rotation
    # -----------------------------
    ...
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title('Before (red) vs After (green) Rotation')

    # Draw axes
    origin = np.zeros((2, 3))
    ax.plot([-2, 2], [0, 0], [0, 0], 'r', label='X-axis')
    ax.plot([0, 0], [-2, 2], [0, 0], 'g', label='Y-axis')
    ax.plot([0, 0], [0, 0], [-2, 2], 'b', label='Z-axis')

    # Plot before/after
    ax.scatter(data_before[:, 0], data_before[:, 1], data_before[:, 2],
               color='red', s=3, alpha=0.5, label='Before rotation')
    ax.scatter(data_after[:, 0], data_after[:, 1], data_after[:, 2],
               color='green', s=3, alpha=0.7, label='After rotation')
